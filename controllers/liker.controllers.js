...
module.exports = {
  getAllLikerControllers: async (req, res) => {
    try {
      ...
      if (!queryParams) {
        result = await n.array(
          n.type({
            ...
            created_at: n.date('L.created_at'),
            updated_at: n.date('L.updated_at')
          })
        )
          .withQuery(
            knex('likers as L')
              .innerJoin('users as U', 'L.user_id', 'U.id')
              .innerJoin('recipes as R', 'L.recipe_id', 'R.id')
          ...
          result = await n.array(
            ...
            .withQuery(
              knex('likers as L')
                ...
                .orderBy(`${queryParams?.orderBy ? `L.${queryParams?.orderBy}` : 'L.id'}`, `${queryParams?.sortBy || 'desc'}`)
  ...
  getLikerByUserIdControllers: async (req, res) => {
    try {
      ...
      if (user.id !== userId) throw new createErrors.UnavailableForLegalReasons('Failed to get likers, your\'e not the liker')
  ...
  postLikerControllers: async (req, res) => {
    try {
      const data = req.body
      ...
      const insertNewLikers = {
        user_id: user.id,
        recipe_id: data.recipe_id
      }
      const result = await knex('likers').insert(insertNewLikers).returning('id')
  ...
  deleteLikerControllers: async (req, res) => {
    try {
      const params = req.params
      const paramsLength = Object.keys(params).length
      const user = req.userData
